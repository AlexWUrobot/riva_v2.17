...
# Pick a host port for gRPC if the default is already taken.
# Container always listens on $riva_speech_api_port; host can map a different port.
riva_host_grpc_port="${RIVA_HOST_GRPC_PORT:-$riva_speech_api_port}"
if ss -ltn 2>/dev/null | awk '{print $4}' | grep -qE ":${riva_host_grpc_port}$"; then
    # Find a free port in a small range.
    riva_host_grpc_port=""
    for p in 50051 50052 50053 50054 50055 50056 50057 50058 50059 50060; do
        if ! ss -ltn 2>/dev/null | awk '{print $4}' | grep -qE ":${p}$"; then
            riva_host_grpc_port="$p"
            break
        fi
    done
fi
if [ -z "$riva_host_grpc_port" ]; then
    echo "Unable to find a free host port to map gRPC port ${riva_speech_api_port}. Set RIVA_HOST_GRPC_PORT or stop the service using it."
    exit 1
fi
if [ "$riva_host_grpc_port" != "$riva_speech_api_port" ]; then
    echo "Host port ${riva_speech_api_port} is busy; mapping ${riva_host_grpc_port}->${riva_speech_api_port} for Riva gRPC."
fi
...
if [ $(docker ps -q -f "name=^/$riva_daemon_speech$" | wc -l) -eq 0 ]; then
    ...
    docker rm -f $riva_daemon_speech &> /dev/null || true
    if [[ $riva_target_gpu_family == "tegra" ]]; then
        # 8888 is commonly used by other services (e.g., mediamtx). Pick a free host port.
        riva_host_8888_port="${RIVA_HOST_8888_PORT:-}"
        if [ -z "$riva_host_8888_port" ]; then
            for p in 8888 8889 8890 8891 8892 8893 8894 8895; do
                if ! ss -ltn 2>/dev/null | awk '{print $4}' | grep -qE ":${p}$"; then
                    riva_host_8888_port="$p"
                    break
                fi
            done
        fi
        if [ -z "$riva_host_8888_port" ]; then
            echo "Unable to find a free host port to map container port 8888. Set RIVA_HOST_8888_PORT or stop the service using it."
            exit 1
        fi
        if [ "$riva_host_8888_port" != "8888" ]; then
            echo "Host port 8888 is busy; mapping ${riva_host_8888_port}->8888 for Riva container."
        fi
        docker_run_args="-p 8000:8000 -p 8001:8001 -p 8002:8002 -p ${riva_host_8888_port}:8888 --device /dev/bus/usb --device /dev/snd"
    else
    ...
    docker run -d \
        ...
        --ipc=host \
        --gpus '"'$gpus_to_use'"' \
        -p ${riva_host_grpc_port}:$riva_speech_api_port \
    ...
    if [ $(docker ps -q -f "name=^/$riva_daemon_speech$" | wc -l) -eq 0 ]; then
        echo "Riva container failed to start. Check logs with: docker logs $riva_daemon_speech"
        exit 1
    fi
else
